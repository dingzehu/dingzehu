<div align="center">

# Hi, I'm Ding-Ze Hu 👋

**Python & Full-Stack Engineer · Hamburg, Germany 🇩🇪**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ding-ze-hu-4b97b016b/)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=githubpages&logoColor=white)](https://dingzehu.github.io/)
[![Profile Views](https://komarev.com/ghpvc/?username=dingzehu&style=flat-square&color=blue)](https://github.com/dingzehu)

</div>

---

### 🧑‍💻 About Me

Software engineer at **[DESY](https://www.desy.de/)** (Deutsches Elektronen-Synchrotron), one of the world's leading particle physics research centres. I build Python-based automation systems, data pipelines, and full-stack tools that help 10+ research teams access and analyse scientific data.

- 🔭 Currently: Full-Stack Software Engineer @ DESY, Hamburg
- 🎓 MSc Computer Science — Georg-August-Universität Göttingen (thesis: in-memory databases for near-real-time data analysis with Redis & MongoDB, **10× performance gain** over InfluxDB)
- 🛠️ Day-to-day: Python · Django · React · Docker · data pipelines
- 🌍 Languages: English (C1) · German (B1) · Mandarin (native)
---

### 🛠️ Languages & Tools

<div>
  <!-- Core backend -->
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/python-original.svg" title="Python" alt="Python" width="40" height="40"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg" title="Django" alt="Django" width="40" height="40"/>&nbsp;
  <!-- Frontend -->
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" title="React" alt="React" width="40" height="40"/>&nbsp;
  <!-- DevOps -->
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" title="Docker" alt="Docker" width="40" height="40"/>&nbsp;
  <!-- Data -->
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/pandas-original-wordmark.svg" title="Pandas" alt="Pandas" width="40" height="40"/>&nbsp;
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/numpy-original-wordmark.svg" title="NumPy" alt="NumPy" width="40" height="40"/>&nbsp;
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/jupyter-original-wordmark.svg" title="Jupyter" alt="Jupyter" width="40" height="40"/>&nbsp;
  <!-- Databases -->
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/redis-original-wordmark.svg" title="Redis" alt="Redis" width="40" height="40"/>&nbsp;
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/mongodb-original-wordmark.svg" title="MongoDB" alt="MongoDB" width="40" height="40"/>&nbsp;
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/mysql-original-wordmark.svg" title="MySQL" alt="MySQL" width="40" height="40"/>&nbsp;
  <!-- Infrastructure -->
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/linux-original.svg" title="Linux" alt="Linux" width="40" height="40"/>&nbsp;
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/git-original-wordmark.svg" title="Git" alt="Git" width="40" height="40"/>&nbsp;
  <img src="https://github.com/dingzehu/dingzehu/blob/main/icon/amazonwebservices-original-wordmark.svg" title="AWS" alt="AWS" width="40" height="40"/>&nbsp;
</div>

---

### 📌 Featured Projects

| Project | Description | Stack |
|---|---|---|
| 🔬 **DESY Data Platform** | Scalable metadata schemas and automation engines for scientific research workflows | Python · Django · React · Docker |
| 📊 **Research Graph Engine** | High-performance scraper + graph embedding pipeline transforming publication metadata into interactive 2D network visualisations | Python · Pandas · NumPy · Node.js |
| ⚡ **In-Memory DB Thesis** | Migrated large-scale environmental datasets to Redis/MongoDB, achieving **10× query performance** over InfluxDB for near-real-time analysis | Python · Redis · MongoDB · InfluxDB |

> *More projects coming soon — currently working on open-source tooling in the AI/data space.*

---

### 📊 My Stats

![GitHub Streak](http://github-readme-streak-stats.herokuapp.com?user=dingzehu&theme=dark&background=000000)
